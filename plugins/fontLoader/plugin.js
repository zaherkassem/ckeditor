...
//Always run once becuase of runnig each time in a new ck editor frame
//            if (FontLoader.testDiv === null) {
            this._runOnce(document);
//            } else {
//                this._loadFonts();
//            }
        },
        _runOnce: function (document) {
            var self = this,
                clonedDiv, j,
            ...
            if (!FontLoader.useAdobeBlank) {
                ...
                if (FontLoader.useIntervalChecking) {
                    ...
                    this._intervalId = window.setInterval(function intervalFire() {
                        self._checkAdobeBlankSize(document);
                    }, this._intervalDelay);
                    this._checkAdobeBlankSize(document);
                } else {
        ...
        _checkAdobeBlankSize: function (document) {
            var adobeBlankDiv = this._testContainer.firstChild;
            this._adobeBlankLoaded(adobeBlankDiv, document);
        },
        _adobeBlankLoaded: function (adobeBlankDiv, document) {
         ...
         */
        sizeWatcherChangedSize: function (sizeWatcher, document) {
            var watchedElement = sizeWatcher.getWatchedElement();
            if (sizeWatcher === this._adobeBlankSizeWatcher) {
                this._adobeBlankLoaded(watchedElement, document);
            } else {
    ...
    function SizeWatcher(element, options) {
        ...
        this._generateScrollWatchers(options.size, options.document);
    ...
    SizeWatcher.prototype = {
        ...
        _generateScrollWatchers: function (size, document) {
        ...
        prepareForWatch: function (document) {
            var parentNode,
        ...
        handleEvent: function (event) {
            ...
            if (this._delegate && typeof this._delegate.sizeWatcherChangedSize === "function") {
                var document = event.path[event.path.length - 1];

                //sanity check that the last node in the event path is the document
                if (document.nodeType !== 9 /** document type --> 9 */) {
                    document = window.document;
                }

                this._delegate.sizeWatcherChangedSize(this, document);
...
(function () {
    CKEDITOR.plugins.add("fontLoader", {
        loadFonts: function (fontFamiliesArray, onError, onSuccess, timeout, editor) {
            timeout = timeout || 3000;

            var fontLoader = new FontLoader(fontFamiliesArray, {
